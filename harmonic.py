...
def chebychev_filters(k=3, groups=1, expand_dim=1, level=None, DC=True, l1_norm=True):
    n = k
...
class Harm2d(nn.Module):
    ...
    def __init__(self, ni, no, kernel_size, stride=1, padding=0, bias=True, dilation=1, use_bn=False, level=None,
        ...
        self.dct = nn.Parameter(
            dct_filters(k=kernel_size, groups=ni if use_bn else 1, expand_dim=1 if use_bn else 0, level=level, DC=DC),
            requires_grad=False)
...
class DeCheb2d(nn.Module):
    ...
    def forward(self, x):
        if not hasattr(self, 'bn'):
            filt = torch.sum(self.weight * self.cheb, dim=2)
            #F.conv_transpose2d
            x = F.conv_transpose2d(x, filt, bias=self.bias, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups)
            return x
        else:
            x = F.conv_transpose2d(x, self.cheb, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=x.size(1))
            x = self.bn(x)
            x = F.conv_transpose2d(x, self.weight, bias=self.bias, padding=0, groups=self.groups)
            return x

class Cheb2d(nn.Module):

    def __init__(self, ni, no, kernel_size, stride=1, padding=0, bias=True, dilation=1, use_bn=False, level=None,
                 DC=True, groups=1, alpha=0.95):
        super(Cheb2d, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
